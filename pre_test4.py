...
def put_time_tag():
    visit = pd.read_hdf('visit_quarter_test.h5')
    ...
    del visit['month']
    del visit['day']
    visit.to_hdf('visit_quarter_test_new.h5',mode='w',key='tt',complevel=6,complib='blosc') #这里需要区分两次的文件，然后在终端合并
...
def get_train_features():
    visit = pd.read_hdf('visit_quarter_test_new.h5')
    ...
    features.to_csv('test_feature4.csv',index=False)
